<?php  
class fcom_webmedios_dynamic_Widget extends WP_Widget {

    function widget( $args, $instance ) {
        extract( $args, EXTR_SKIP );
        echo $before_widget;
        $this->fcom_display_widgets_front($instance);
        echo $after_widget;
      }
      ...
      function form( $instance ) {
        ...
        ?>
          ...
          <div class="dw-widget-extends" data-setting="#<?php echo $this->get_field_id('widgets') ?>" >
            <p class="description"><?php _e('Arrastrar widgets en esta área','fcom_minion') ?></p>
            <?php
              ...
              if( !empty($widgets) && is_array($widgets) ){
                ...
                foreach ($widgets as $widget) {
                  if( !empty( $widget ) ) {
                    ...
                    parse_str($widget,$s);
                    $this->fcom_display_widgets($s, $number);
                  }
                  $number++;
                }
              }
      ...
      function fcom_get_widgets( $id_base, $number ){
        global $wp_registered_widgets;
      ...
      function fcom_display_widgets($s, $number){
        $instance = !empty($s['widget-'.$s['id_base']]) ? array_shift( $s['widget-'.$s['id_base']] ) : array();
        $widget = $this->fcom_get_widgets( $s['id_base'], $number );
      ?>  
        <?php if( $widget ) { ?>
          ...
          <div class="widget-inside">
            <div class="widget-content">
              <?php if( isset($s['id_base'] ) ) { 
                ...
                echo "\t\t<p>" . __('There are no options for this widget.','fcom_minion') . "</p>\n"; 
              } ?>
            </div>
            ...
            <div class="widget-control-actions">
              <div class="alignleft">
                <a class="widget-control-remove" href="#remove"><?php _e('Delete','fcom_minion'); ?></a> |
                <a class="widget-control-close" href="#close"><?php _e('Close','fcom_minion'); ?></a>
              </div>
              <div class="alignright widget-control-noform">
                <?php submit_button( __( 'Save', 'fcom_minion' ), 'button-primary widget-control-save right', 'savewidget', false, array( 'id' => 'widget-' . esc_attr( $s['widget-id'] ) . '-savewidget' ) ); ?>
